import 'package:flutter/material.dart';

class Coloors {
  Coloors._();

  static const Color orangeDark = Color(0xFFFF5A1F); /* FF6933 */
  static const Color orangeLight = Color(0xFFFF4B0A);

  static const Color cambridgeBlueLight = Color(0xFF5B8677);
  static const Color cambridgeBlueDark = Color(0xFF6D9C8C);

  static const Color redLight = Color(0xFFFF0000);
  static const Color redDark = Color(0xFFFF0000);

  static const Color greenDark = Color(0xFF00A884);
  static const Color greenLight = Color(0xFF008069);
  static const Color blueDark = Color(0xFF53BDEB);
  static const Color blueLight = Color(0xFF027EB5);
  static const Color greyDark = Color(0xFF8696A0);
  static const Color greyLight = Color(0xFF667781);
  static const Color backgroundDark = Color(0xFF111B21);
  static const Color backgroundLight = Color(0xFFF6F6F6);
  static const Color greyBackground = Color(0xFF202C33);

  static const Color padsyColor = Color(0xFF00786A);
}
